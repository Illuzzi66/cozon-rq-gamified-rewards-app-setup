...
- [x] Withdrawal approval system
  ...
  - [x] Integrate Paystack for automatic payment processing
    - [x] Store Paystack API key securely
    ...
    - [x] Add webhook handler for Paystack transfer status updates
    - [x] Implement retry mechanism for failed payments
    - [x] Add email notifications for payment status updates
    - [x] Deploy email notification edge function
    - [x] Integrate email service with webhook handler
    - [ ] Configure webhook URL in Paystack dashboard
    - [ ] Create payment monitoring dashboard for admins
    - [ ] Add retry statistics and failure analytics
...
- [x] Remove error alerts from all pages
  ...
  - [x] Premium page
  - [x] Profile page
- [x] Fix slow boot performance issue
  - [x] Remove aggressive setInterval overlay removal (100ms polling)
  - [x] Simplify error overlay blocking logic (removed customElements overrides)
  - [x] Optimize AuthContext initialization (set loading false immediately)
  - [x] Review and optimize initial data loading (defer non-critical operations)
  - [x] Fix coin_balance reference in Layout component
- [x] Update spin wheel reward display format
